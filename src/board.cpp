...
Board::Board(std::array<int, 64> state, std::array<uint64_t, 15> bitboards, bool whiteTurn, int castlingRights, int enPassantSquare) {
  state = state;
  bitboards = bitboards;
  whiteTurn = whiteTurn;
...
std::array<int, 64> Board::getState() {
  return state;
}

std::set<Move> Board::getMoves() {
  return moves;
}
...
void Board::convertFromFen(std::string fenString) {
  std::map<char, int> pieceLetterToPieceNum = {
    {'p', 1},
    {'n', 2},
    {'b', 3},
    {'r', 4},
...
void Board::generatePawnMoves() {
  ...
  if (whiteTurn) {
    uint64_t westCaptures = bitboards[colourValue + 1] << 7 & (bitboards[8] | (1ULL << enPassantSquare)) & 0x7f7f7f7f7f7f7f7f;
    addMovesFromBitmap(westCaptures, -7);

    uint64_t eastCaptures = bitboards[colourValue + 1] << 9 & (bitboards[8] | (1ULL << enPassantSquare)) & 0xfefefefefefefefe;
    addMovesFromBitmap(eastCaptures, -9);

    uint64_t forwardMoves = bitboards[colourValue + 1] << 8 & ~(bitboards[0] | bitboards[8]);
    ...
    uint64_t westCaptures = bitboards[colourValue + 1] >> 9 & (bitboards[0] | (1ULL << enPassantSquare)) & 0x7f7f7f7f7f7f7f7f;
    addMovesFromBitmap(westCaptures, 9);

    uint64_t eastCaptures = bitboards[colourValue + 1] >> 7 & (bitboards[0] | (1ULL << enPassantSquare)) & 0xfefefefefefefefe; 
    ...
    uint64_t doublePawnMoves = 
      (unmovedPawns >> 8 & freeSquares) >> 8 & (unmovedPawns >> 16 & freeSquares);
    addMovesFromBitmap(doublePawnMoves, 16);
  }
}
...
void Board::generateKingMoves() {
  ...
  while (bitboardCopy) {
    int pieceSquare = std::countr_zero(bitboardCopy);
    uint64_t possibleMoves = kingMoveMasks[pieceSquare] & ~bitboards[colourValue];
    while (possibleMoves) {
      int destinationSquare = std::countr_zero(possibleMoves);
      int flags = 0;
      if (state[destinationSquare]) {
